...
class zyd_pathSim_algo:
    ...
    def start(self,gene1,gene2,max_length):
        ...
        meta_paths_from_gene1,meta_path_from_gene2=self.find_meta_paths(gene1,gene2,graph,max_step)
        listxx=list(set(self.list1).intersection(set(self.list2)))
        print(listxx)
        if len(listxx)==0:
            print("there is no meta paths between " + gene1 + " and " + gene2)
            return []
    ...
    def is_connected(self,gene1,gene2,graph,max_step):
        ...
        try:
            sp = nx.shortest_path_length(graph, gene1, gene2)
        except:
            return False
